...
The result is equivalent to saying that \textit{locally}, the solutions of our system of differential equations given by $\nabla$ is a complex vector space, with dimension equal to the rank of the system. One can then think of the Cauchy-Lipschitz (or Picard–Lindelöf) theorem. It indeed works when $X$ is a curve because we have locally an ordinary differential equation. If $n>1$, this theorem cannot be apply.\\
There is an alternative for this type of system : the Frobenius theorem.

\begin{mylemme}
Let $x \in X$.\\
There exists an open set $U$ containing $x$ such that the evaluation $\text{ev}_x : \Ker(\nabla|_U) \to \pi^{-1}(x)$ is an isomorphism (of sheaves).
\end{mylemme}
